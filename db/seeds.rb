...
cake_citron_graines_pavot    = Recipe.create!(photo: File.open(Rails.root.join('db/fixtures/recipes/cake_citron_pavot.jpg')), title: "Cake au citron et graines de pavot", subtitle: "originale", description: "Aujourd'hui je vous propose la recette d'un cake extraordinaire! Un délicieux cake au citron bien moelleux, humide, bien parfumé et pas trop sucré. Bref c'est une véritable tuerie, après le cake anglais aux fruits confits, c'est mon 2e cake préféré! ", prep_time: "10 min", cook_time: "55 min", rest_time: "12 h", kind: "original", published: true, user: diane, published_on: "2019-03-23", servings: 4, upvote: 5)
tarte_au_fraise              = Recipe.create!(photo: File.open(Rails.root.join('db/fixtures/recipes/tarte_aux_fraises.jpg')), title: "Tarte aux fraises", description: "Une pâte brisée maison, une bonne crème pâtissière à la vanille et des belles fraises, c’est tout ce qu’il vous faut pour réaliser cette recette de tarte aux fraises facile à la maison. Réutilisez cette base de tarte pour décliner en tarte aux framboises, aux mûres, aux myrtilles… ou tout autre fruit qu’il vaut mieux manger cru que cuit au four.", prep_time: "10 min", cook_time: "55 min", rest_time: "12 h", user: thierry, kind: "original", published: true, published_on: "2019-03-15", servings: 6, upvote: 7)
tajine_poulet_citron_confits = Recipe.create!(photo: File.open(Rails.root.join('db/fixtures/recipes/tajine_poulet_citrons_confits.jpg')), title: "Tajine de poulet aux citrons confits ", description: "Encore une recette réalisée lors de mon périple à Marrakech. C’était GENIAL. J’ai adoré cette recette de tajine, facile à réaliser, pleine de saveurs et de parfums. A faire et à refaire.", prep_time: "10 min", cook_time: "55 min", rest_time: "12 h", user: melanie, kind: "original", published: true, published_on: "2019-03-12", servings: 4, upvote: 10)
curry_de_crevettes_doux      = Recipe.create!(photo: File.open(Rails.root.join('db/fixtures/recipes/curry_de_crevettes_doux.jpg')), title: "Curry de crevettes doux", description: "J’ai découvert cette recette lors de mon dernier voyage en asie. Ce curry est extrêmement riche en protéines. Il est crémeux, savoureux, légèrement épicé et ne nécessite que 20 minutes de préparation. Rapide à préparer, fait maison et beaucoup plus sain que les plats à emporter. ", prep_time: "10 min", cook_time: "55 min", rest_time: "12 h", user: diane, kind: "original", published: true, published_on: "2019-03-29", servings: 4, upvote: 7)
muesli_suisse                = Recipe.create!(photo: File.open(Rails.root.join('db/fixtures/recipes/muesli_suisse.jpg')), title: "Muesli Suisse", description: "Très prisé des sportifs, ce muesli riche en fibres. Etant donné qu'elles macèrent dans votre estomac, les graines de lin doivent toujours être consommées avec suffisamment de liquide. La consommation de graines de lin est particulièrement recommandée le matin, car elles stimulent le métabolisme et la digestion.", prep_time: "10 min", cook_time: "5 min", rest_time: "12 h", user: thierry, kind: "original", published: true, published_on: "2019-03-29", servings: 4, upvote:9)
macarons_a_la_noix_de_coco   = Recipe.create!(photo: File.open(Rails.root.join('db/fixtures/recipes/macarons_a_la_noix_de_coco.jpg')), title: "Macarons à la noix de coco", description: "Les macarons de noix de coco sont une vrai bombe de protéines. Ils sont parfaitement adaptées en tant que snakc après vos sessions de sport intensives, mais aussi à tout moment de la journée. De plus, la graisse contenue dans la noix de coco est censée avoir un effet positif sur le cerveau.", prep_time: "30 min", cook_time: "25 min", rest_time: "2 h", user: thierry, kind: "original", published: true, published_on: "2019-03-18", servings: 24, upvote: 3)
muesli_au_melange_de_baies   = Recipe.create!(photo: File.open(Rails.root.join('db/fixtures/recipes/muesli_au_melange_de_baies.jpg')), title: "Muesli au mélange de baies", description: "Ce muesli au goût très frais si les baies sont congelés et décongèlent lentement, doit être préparer idéalement la veille au soir en le laissant tremper toute la nuit. dans le cas où le muesli serait trop sec, ajouter du lait et mélanger.", prep_time: "10 min", cook_time: "5 min", rest_time: "8 h", user: thierry, kind: "original", published: true, published_on: "2019-03-24", servings: 1, upvote: 8)
langue_de_boeuf              = Recipe.create!(photo: File.open(Rails.root.join('db/fixtures/recipes/langue_de_boeuf.jpg')), title: "Langue de boeuf à la sauce madère", description: "La langue de bœuf... beaucoup d'entre vous diront « Beurk j'aime pas » ! Mais bien cuisinée, ce plat principal est un vrai régal. Facile à réaliser, elle demande un peu de temps de préparation et de cuisson, mais on vous l'assure, ça en vaut le coup ! Une bonne sauce bien onctueuse et gourmande à souhait à base du célèbre vin rouge liquoreux, une langue bien cuite, quelques légumes, la langue de bœuf sauce à la Madère est vraiment à essayer!", prep_time: "10 min", cook_time: "2 h", rest_time: "6 h", user: melanie, kind: "original", published: false, published_on: "2019-03-14", servings: 4, upvote: 7)
...
Dose.create!(step: muesli_suisse_step_1, ingredient: amandes_hachées, quantity: 30, unit: 'g')
...
Dose.create!(step: macarons_a_la_noix_de_coco_step_2, ingredient: pâte_de_noix_de_cajou, quantity: 1, unit: 'cuillère à café')
...
Dose.create!(step: curry_de_crevettes_doux_step_3, ingredient: yaourt_à_la_grecque, quantity: 1)
...
Dose.create!(step: muesli_au_melange_de_baies_step_3, ingredient: yaourt_à_la_grecque, quantity: 250, unit:'g')
...
Dose.create!(step: muesli_suisse_step_4, ingredient: yaourt_à_la_grecque_allege, quantity: 250, unit: 'g')
...
StepUtensil.create!(step: muesli_suisse_step_1, utensil: poêle)
...
StepUtensil.create!(step: muesli_au_melange_de_baies_step_1, utensil: poêle)
...
StepUtensil.create!(step: macarons_a_la_noix_de_coco_step_3, utensil: papier_sulfurisé)
StepUtensil.create!(step: macarons_a_la_noix_de_coco_step_3, utensil: plaque_pâtisserie)
